...
export default function EmailScanner({ onStateUpdate }: { onStateUpdate: () => void }) {
  ...
  const analyzeNow = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/analyze`, {
        ...
        body: JSON.stringify({ kind: "email", content: inputText }),
      });
    ...
    <div className="scanner-page">
      <div className="scanner-left">
        <div className="analyzer-card">
          <h2 className="analyzer-card-title">Analyze Email</h2>
          <textarea
          ...
          {analyzing && (
            <div className="scan-progress">
              ...
              <p className="scan-progress-label">Running heuristics & ML classifier...</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="scanner-right">
        ...
        {!result && !analyzing && (
          <div className="card" style={{ padding: "3rem 1.5rem", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "0.84rem", color: "#64748B" }}>Paste an email on the left and click Analyze to see results here.</p>
          </div>
        )}
      </div>
    </div>
